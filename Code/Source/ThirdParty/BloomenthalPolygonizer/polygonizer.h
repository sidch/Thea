...
namespace BloomenthalPolygonizer {
...
class Polygonizer
{
 ...
 public:
  ...
  /** Return number of normals generated after the polygonization.
      ...
      no_vertices.
      Call this function only when march has been called. */
  std::size_t no_normals() const
  {
    return (int)gnormals.size();
  }

  /// Return triangle with index i.
  TRIANGLE& get_triangle(std::size_t i)
  {
    return gtriangles[i];
  }

  /// Return vertex with index i.
  VERTEX& get_vertex(std::size_t i)
  {
    return gvertices[i];
  }

  /// Return normal with index i.
  NORMAL& get_normal(std::size_t i)
  {
    return gnormals[i];
  }
};
