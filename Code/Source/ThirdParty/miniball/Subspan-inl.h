...
namespace SEB_NAMESPACE {
  ...
  Subspan<D, Float, Pt, PointAccessor>::Subspan(const PointAccessor& S, int index)
  : S(S), membership(S.size()), members(D+1)
  {
  ...
  template<unsigned int D, typename Float, class Pt, class PointAccessor>
  Subspan<D, Float, Pt, PointAccessor>::~Subspan()
  {
    for (unsigned int i=0; i<D; ++i) {
      delete[] Q[i];
      delete[] R[i];
  ...
  template<unsigned int D, typename Float, class Pt, class PointAccessor>
  void Subspan<D, Float, Pt, PointAccessor>::add_point(int index) {
  ...
  template<unsigned int D, typename Float, class Pt, class PointAccessor>
  void Subspan<D, Float, Pt, PointAccessor>::remove_point(const unsigned int local_index) {
  ...
  Float Subspan<D, Float, Pt, PointAccessor>::
                          ...
                          RandomAccessIterator2 w)
  {
    ...
    for (unsigned int j = 0; j < r; ++j) {
      const Float scale = inner_product(w,w+D,Q[j],Float(0));
      for (unsigned int i = 0; i < D; ++i)
        w[i] -= scale * Q[j][i];
    }

    return inner_product(w,w+D,w,Float(0));
  }

  template<unsigned int D, typename Float, class Pt, class PointAccessor>
  Float Subspan<D, Float, Pt, PointAccessor>::representation_error()
  {
    using std::abs;
  ...
  template<typename RandomAccessIterator1,
  typename RandomAccessIterator2>
  void Subspan<D, Float, Pt, PointAccessor>::
  ...
  template<unsigned int D, typename Float, class Pt, class PointAccessor>
  void Subspan<D, Float, Pt, PointAccessor>::append_column()
  ...
  // Precondition: r<D
  {
    ...
    //  zero all entries R[r][D-1] down to R[r][r+1]
    for (unsigned int j = D-1; j > r; --j) {
  ...
  template<unsigned int D, typename Float, class Pt, class PointAccessor>
  void Subspan<D, Float, Pt, PointAccessor>::hessenberg_clear (unsigned int pos)
  ...
  template<unsigned int D, typename Float, class Pt, class PointAccessor>
  void Subspan<D, Float, Pt, PointAccessor>::special_rank_1_update ()
