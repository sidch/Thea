...
namespace SEB_NAMESPACE {
  ...
  // template arguments:
  // Float must be a floating point data type for which * / - + are defined (SC: use only double)
  ...
  template<unsigned int D, typename Float, class Pt = Point<D, Float>, class PointAccessor = std::vector<Pt> >
  ...
  public: // construction and destruction:
    ...
    // set S.
    : S(P), up_to_date(true), support(NULL)
    {
      allocate_resources();
      SEB_ASSERT(!is_empty());
      update();
  ...
  private: // member fields:
    ...
    Subspan<D, Float, Pt, PointAccessor> *support;          // the points that lie on the current
