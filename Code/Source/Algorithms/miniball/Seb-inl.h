...
namespace SEB_NAMESPACE {
  ...
  template<unsigned int D, typename Float, class Pt, class PointAccessor>
  void Smallest_enclosing_ball<D, Float, Pt, PointAccessor>::allocate_resources()
  {
    center            = new Float[D];
    center_to_aff     = new Float[D];
    center_to_point   = new Float[D];
    lambdas           = new Float[D+1];
  }

  template<unsigned int D, typename Float, class Pt, class PointAccessor>
  void Smallest_enclosing_ball<D, Float, Pt, PointAccessor>::deallocate_resources()
  {
    delete[] center;
  ...
  template<unsigned int D, typename Float, class Pt, class PointAccessor>
  void Smallest_enclosing_ball<D, Float, Pt, PointAccessor>::init_ball()
  // Precondition: |S| > 0
  // Sets up the search ball with an arbitrary point of S as center
  // and with exactly one of the points farthest from center in
  ...
  // most twice as large as the minball.
  {
    ...
    if (support != NULL)
      delete support;
    support = new Subspan<D, Float, Pt, PointAccessor>(S,farthest);
  ...
  template<unsigned int D, typename Float, class Pt, class PointAccessor>
  bool Smallest_enclosing_ball<D, Float, Pt, PointAccessor>::successful_drop()
  ...
  template<unsigned int D, typename Float, class Pt, class PointAccessor>
  Float Smallest_enclosing_ball<D, Float, Pt, PointAccessor>::find_stop_fraction(int& stopper)
  ...
  // and to -1 if no such point was found.
  {
    ...
    for (unsigned int j = 0; j < S.size(); ++j)
      if (!support->is_member(j)) {
        ...
        = inner_product(center_to_aff,center_to_aff+D,
        ...
        bound -= inner_product(center_to_point,center_to_point+D,
                               center_to_point,Float(0));
        bound /= 2 * (dist_to_aff_square - dir_point_prod);

        // Infinite loop fix from Paul Harris
        // https://github.com/hbf/miniball/pull/13
        //
  ...
  template<unsigned int D, typename Float, class Pt, class PointAccessor>
  void Smallest_enclosing_ball<D, Float, Pt, PointAccessor>::update()
  ...
  template<unsigned int D, typename Float, class Pt, class PointAccessor>
  void Smallest_enclosing_ball<D, Float, Pt, PointAccessor>::verify()
  {
    using std::inner_product;
    using std::abs;
    using std::cout;
    ...
    for (unsigned int k = 0; k < S.size(); ++k) {
      ...
      ball_error = sqrt(inner_product(center_to_point,center_to_point+D,
                                      center_to_point,Float(0)))
      - radius_;
  ...
  template<unsigned int D, typename Float, class Pt, class PointAccessor>
  void Smallest_enclosing_ball<D, Float, Pt, PointAccessor>::test_affine_stuff()
  {
    using std::cout;
    using std::endl;
    ...
    cout << S.size() << " points in " << D << " dimensions" << endl;

    if (!is_empty()) {
      support = new Subspan<D,Float,PointAccessor,Pt>(S,0);
      cout << "initializing affine subspace with point S[0]" << endl;

      direction = new Float[D];
    }
    else return;
  ...
  template<unsigned int D, typename Float, class Pt, class PointAccessor>
  const Float Smallest_enclosing_ball<D, Float, Pt, PointAccessor>::Eps = Float(1e-14);
