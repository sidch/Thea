...
class BoundedSortedArrayN
{
  ...
  public:
    ...
    T const & front() const
    {
      theaAssertM(num_elems > 0, "BoundedSortedArrayN: Can't get first element of empty array");
      return values[0];
    }
    ...
    T const & back() const
    {
      theaAssertM(num_elems > 0, "BoundedSortedArrayN: Can't get last element of empty array");
    ...
    {
      theaAssertM(i < num_elems, format("BoundedSortedArrayN: Index %ld out of bounds [0, %ld)", (long)i, (long)num_elems));
      return values[i];
    }
